...
<head>
    ...
    <title>ALANHAR | نظام الصرافة المتقدم - ديناميكي ذكي</title>
    ...
    <style>
        * {
        ...
        h1 { font-size: 2rem; letter-spacing: -0.3px; }
        .tabs {
            display: flex;
            background: white;
            border-radius: 48px;
        ...
        .content-section { display: none; animation: fade 0.25s ease; }
        ...
        footer { text-align: center; margin-top: 40px; color: #5b6e8c; font-size: 0.8rem; }
        @media print {
            .print-friendly {
                margin: 0;
                padding: 0;
...
<div class="container">
    <header>
        <div class="logo"><i class="fas fa-chart-line"></i><div><h1>نظام ALANHAR - الصرافة الذكية</h1><div>إدارة احترافية للدولار - اليورو - الليرة</div><div>Profesyonel Döviz Yönetimi | USD | EUR | TRY</div></div></div>
    </header>
    ...
    <div id="dailyInput" class="content-section active">
        ...
        <div class="actions no-print" style="display: flex; gap: 16px; margin-top: 20px;"><button id="saveDailyDataBtn" class="btn btn-success"><i class="fas fa-save"></i> حفظ اليوم</button><button id="resetDailyBtn" class="btn btn-primary"><i class="fas fa-undo-alt"></i> إعادة تعيين</button><button id="viewReportBtn" class="btn"><i class="fas fa-eye"></i> عرض التقرير اليومي</button></div>
    </div>

    <!-- التقرير اليومي (طباعة جميلة) -->
    <div id="dailyReport" class="content-section"><div class="card"><div class="card-title"><i class="fas fa-print"></i> التقرير اليومي المفصل</div><div id="dailyReportContainer" class="print-friendly"></div></div><div class="actions no-print"><button id="printDailyBtn" class="btn btn-success"><i class="fas fa-print"></i> طباعة التقرير</button><button id="backToDailyBtn" class="btn">رجوع</button></div></div>
    ...
    <!-- التقرير الشهري بمربعات الأرباح والمصروفات + صلج -->
    <div id="monthlyTab" class="content-section"><div class="card"><div class="card-title"><i class="fas fa-chart-pie"></i> التقرير الشهري - Aylık Rapor</div><div class="input-group" style="max-width: 260px;"><label>اختر الشهر</label><input type="month" id="monthPicker"></div><button id="loadMonthlyReportBtn" class="btn">عرض التقرير</button>
        ...
        <!-- صلج في الشهري -->
        <div style="margin-top: 25px;"><h3><i class="fas fa-landmark"></i> حساب الصلج (Saljoğ) الشهري / دولار & يورو</h3><div id="monthlySaljogBox" class="saljog-grid"></div></div>
    ...
    <footer class="no-print">ALANHAR Sistemi | Döviz & Havale Yönetimi | تحديث فوري</footer>
</div>
...
<script>
    ...
    let customExpenses = [];
    let allHistory = JSON.parse(localStorage.getItem("ALANHAR_MASTER_DATA")) || [];

    function saveMasterData() { localStorage.setItem("ALANHAR_MASTER_DATA", JSON.stringify(allHistory)); }
    function saveExpensesStructure() { localStorage.setItem("ALANHAR_EXP_STRUCT", JSON.stringify({ defaultExpenses, customExpenses })); }
    ...
    function attachProfitEvents() { document.querySelectorAll(".profit-input-val").forEach(inp => inp.addEventListener("input", updateTotalsFromUI)); }
    ...
    function renderExpenses() {
        ...
        document.querySelectorAll(".expense-amount-field").forEach(inp => inp.addEventListener("input", updateTotalsFromUI));
        document.querySelectorAll(".expense-curr-field").forEach(sel => sel.addEventListener("change", updateTotalsFromUI));
        document.querySelectorAll(".removeExpenseBtn").forEach(btn => btn.addEventListener("click", (e) => { let id = btn.dataset.id; customExpenses = customExpenses.filter(c => c.id != id); defaultExpenses = defaultExpenses.filter(d => d.id != id); saveExpensesStructure(); renderExpenses(); updateTotalsFromUI(); }));
        updateTotalsFromUI();
    }
    document.getElementById("addExpenseItem")?.addEventListener("click", () => {
        let ar = prompt("اسم المصروف بالعربية");
        let tr = prompt("Türkçe isim");
        if(ar && tr) { customExpenses.push({ id: "cust_"+Date.now(), nameAr: ar, nameTr: tr, amount: 0, currency: "USD" }); saveExpensesStructure(); renderExpenses(); updateTotalsFromUI(); }
    });
    ...
    function collectCurrentData() {
        ...
        let allExp = [...defaultExpenses, ...customExpenses];
        let expensesArr = allExp.map(e => ({ nameAr: e.nameAr, nameTr: e.nameTr, amount: e.amount, currency: e.currency }));
        return {
    ...
    function renderHistoryTable() {
        ...
        document.querySelectorAll(".delHistoryBtn").forEach(btn => btn.addEventListener("click", (e) => { let idx = btn.dataset.idx; allHistory.splice(idx,1); saveMasterData(); renderHistoryTable(); }));
    }
    function loadRecordToEdit(rec) {
        ...
        // صلج لا يمكن استرجاع التفاصيل الكاملة لكن نعرض مجموع تقريبي
    ...
    // عرض التقرير اليومي بشكل جميل
    function displayDailyReport() {
        let d = collectCurrentData();
        let profitHtml = `<div class="profits-dashboard">`;
        for(let cat of profitCategories){
            let u = d.profits[cat.id]?.usd||0, e = d.profits[cat.id]?.eur||0;
            if(u!==0 || e!==0) profitHtml += `<div class="profit-card"><h4>${cat.nameAr}</h4><div>$ ${u.toFixed(2)} &nbsp;&nbsp; € ${e.toFixed(2)}</div></div>`;
        }
        profitHtml += `</div>`;
        ...
        expHtml += `</div>`;
        let totalProfSum = Object.values(d.profits).reduce((s,p)=>s+p.usd,0);
        let totalEurSum = Object.values(d.profits).reduce((s,p)=>s+p.eur,0);
        document.getElementById("dailyReportContainer").innerHTML = `
            <div class="summary-stat" style="display:grid; grid-template-columns:repeat(auto-fit,minmax(200px,1fr)); gap:15px; margin-bottom:25px;">
                <div class="summary-stat">🇺🇸 فائض دولار: $${d.usd.surplus.toFixed(2)}</div>
                <div class="summary-stat">🇪🇺 فائض يورو: €${d.eur.surplus.toFixed(2)}</div>
            ...
            <h3>🏆 الأرباح التفصيلية</h3>${profitHtml}<div class="total-badge">إجمالي الأرباح: $${totalProfSum.toFixed(2)} / €${totalEurSum.toFixed(2)}</div>
            <h3 style="margin-top:25px;">📋 المصروفات</h3>${expHtml}<div class="total-badge">إجمالي المصروفات: ${d.expenses.filter(e=>e.currency==='USD').reduce((s,e)=>s+e.amount,0).toFixed(2)} $ / ${d.expenses.filter(e=>e.currency==='EUR').reduce((s,e)=>s+e.amount,0).toFixed(2)} € / ${d.expenses.filter(e=>e.currency==='TRY').reduce((s,e)=>s+e.amount,0).toFixed(2)} ₺</div>
        `;
        document.querySelector(".tab[data-tab='dailyReport']").click();
    }

    // التقرير الشهري مع صلج
    function buildMonthly() {
        ...
        let profitsHtml = ""; for(let cat of profitCategories){ let u=aggProfits[cat.id].usd, e=aggProfits[cat.id].eur; if(u!==0||e!==0) profitsHtml += `<div class="profit-card"><h4>${cat.nameAr}</h4><div>USD: $${u.toFixed(2)} | EUR: €${e.toFixed(2)}</div></div>`; }
        if(!profitsHtml) profitsHtml = "<div class='card'>لا توجد أرباح</div>";
        ...
        if(!expHtml) expHtml = "<div class='card'>لا توجد مصروفات</div>";
        document.getElementById("monthlyExpensesGrid").innerHTML = expHtml;
    ...
    document.getElementById("resetDailyBtn")?.addEventListener("click", () => { if(confirm("مسح اليوم")) location.reload(); });
    ...
    document.getElementById("printDailyBtn")?.addEventListener("click", () => { displayDailyReport(); setTimeout(()=>window.print(), 300); });
    document.getElementById("printMonthlyReportBtn")?.addEventListener("click", () => { buildMonthly(); setTimeout(()=>window.print(), 300); });
    document.getElementById("loadMonthlyReportBtn")?.addEventListener("click", buildMonthly);
    document.getElementById("clearAllHistoryBtn")?.addEventListener("click", () => { if(confirm("مسح الكل")){ allHistory=[]; saveMasterData(); renderHistoryTable(); } });
    ...
    renderProfits(); renderExpenses(); updateTotalsFromUI(); renderHistoryTable();
</script>
</body>
</html>
